...
public class AdviceRepositoryImp implements AdviceRepository {
	...
	public List<Map<String, Object>> search(String word) {
		try {
			...
			// 검색 쿼리 구성
			searchSourceBuilder.query(QueryBuilders.boolQuery().must(QueryBuilders.matchQuery("counselcontent", word))
					.should(QueryBuilders.matchPhraseQuery("counselcontent", word)));
