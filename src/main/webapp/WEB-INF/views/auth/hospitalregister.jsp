				...
				<div class="row">
					...
					<div class="col-xl-12">
						<div class="card shadow mb-5" style="margin: 0 auto; width: 80%;">
							...
							<div class="card-body">
								<form method="POST" action="/apa/auth/hospitalregisterok.do"
									name="hospitalJoin">
									<div id="my-info">
										<div id="my-info-name">
										...
										<!-- 	<div class="my-info-input-child">
											...
											<input type="hidden" name="auth" value="hospital">
										</div>
									</div>
	...
	<script>
 		alert("script start"); 
            function checkAll() {
                ...
                else if (!checkPassword($('input[name=hospitalId]').val(), $('input[name=hospitalPw]').val(), $('input[name=pwchecked]').val())) {
                    return false;
                    
                } 
                
                else if (!checkMail($('input[name=hospitalEmail]').val())) {
                    return false;
                
                } 
                
                else if (!checkName($('input[name=hospitalName]').val())) {
                    return false;
            ...
            function checkUserId(hospitalId) {
                ...
                if (!idRegExp.test(hospitalId)) {
                    alert("아이디는 영문 소문자와 숫자 4~12자리로 입력해야합니다!");
                    $('input[name=hospitalId]').val("");
                    $('input[name="hospitalId"]').focus();
                    return false;
                }
                return true;
            }
             ...
             function checkPassword(hospitalId, hospitalPw, pwchecked) {
                ...
                if (!pwRegExp.test(hospitalPw)) {
                    alert("비밀번호는 영문 대소문자와 특수문자, 숫자 4~16자리로 입력해야합니다!");
                    $('input[name=hospitalPw]').val("");
                    $('input[name="hospitalPw"]').focus();
                    return false;
                }

                if (hospitalPw != pwchecked) {
            ...
            function checkMail(hospitalEmail) {
                ...
                if (!emailRegExp.test(hospitalEmail)) {
                    alert("이메일 형식이 올바르지 않습니다!");
                    $('input[name=hospitalEmail]').val("");
                    $('input[name="hospitalEmail"]').focus();
                    return false;
                }
                return true;
            ...
            function checkName(hospitalName) {
                ...
                if (!nameRegExp.test(hospitalName)) {
                    alert("이름이 올바르지 않습니다.");
	...
	<script>
		    ...
		    new daum.Postcode({
		        oncomplete: function(data) {
		        	...
		        	document.hospitalJoin.addr1.value = fullAddr;
		        	document.hospitalJoin.zipcode.value = data.zonecode;
		        	document.hospitalJoin.addr2.focus();
		        }
		    }).open();
